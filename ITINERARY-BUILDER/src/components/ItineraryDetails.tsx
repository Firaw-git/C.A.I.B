...
const ItineraryDetails: React.FC = () => {
  ...
  const [selectedMarkers, setSelectedMarkers] = useState<google.maps.LatLngLiteral[]>([]);
...
if (userId) {
  ...
  if (feedback) {
    ...
    setFeedbackSubmitted(true);
      ...
      map.data.addListener("mouseover", (event: any) => {
        const provinceName = event.feature.getProperty("NAME_1") || event.feature.getProperty("name");
        ...
        infoWindow.setContent(`
          <div style="
            background-color: rgba(126, 122, 122, 0.2);
            padding: 8px 12px;
            border-radius: 8px;
            font-weight: bold;
            color:rgb(32, 31, 94);
      ...
      if (allLatLngs.length > 1) {
        ...
        await computeTotalTripDistanceTime(allLatLngs);
      }
    };

    fetchAndRenderMap();
  }, [itineraryId, selectedMarkers]);
  ...
  const computeTotalTripDistanceTime = async (allLatLngs: google.maps.LatLngLiteral[]) => {
    const url = `https://routes.googleapis.com/distanceMatrix/v2:computeRouteMatrix`;

    const body = {
      origins: allLatLngs.slice(0, -1).map(p => ({
        waypoint: { location: { latLng: { latitude: p.lat, longitude: p.lng } } }
      })),
      destinations: allLatLngs.slice(1).map(p => ({
        waypoint: { location: { latLng: { latitude: p.lat, longitude: p.lng } } }
      })),
      travelMode: "DRIVE",
      units: "METRIC",
  ...
  return (
    ...
    <div className="itinerary-details-page">
      ...
      {distanceText && <div className="segment-distance">🧭 Segment Distance: {distanceText}</div>}
